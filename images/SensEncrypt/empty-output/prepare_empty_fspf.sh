...
do
  echo "$line" | awk '{print "export SENSENCRYPT_FSPF_TAG="substr($0,64,32)}' > .env1
  echo "$line" | awk '{print "export SENSENCRYPT_FSPF_KEY="substr($0,102,64)}' >> .env1
done < "empty-dir/tag_key.txt"

while IFS= read -r line
do
  echo "$line" | awk '{print "SENSENCRYPT_FSPF_TAG="substr($0,64,32)}' > .env
  echo "$line" | awk '{print "SENSENCRYPT_FSPF_KEY="substr($0,102,64)}' >> .env
done < "empty-dir/tag_key.txt"

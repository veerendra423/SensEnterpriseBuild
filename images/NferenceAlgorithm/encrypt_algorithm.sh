...
docker build . -t sensoriant.azurecr.io/nference/algorithm-base:test -f Dockerfile.base
echo ""

echo "Now, we enter the base image to record the trusted state of the python"
echo "dependencies and encrypt the application."
docker run -it --rm --privileged  --device /dev/isgx --device /dev/gsgx  -e SCONE_VERSION=1 -e SCONE_MODE=sim  -e SCONE_VERSION=1 -v `pwd`:/algorithm sensoriant.azurecr.io/nference/algorithm-base:test sh -c "
    cd /algorithm && \

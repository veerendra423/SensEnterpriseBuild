...
int copy_file(const char* from, const char* to) {
    ...
    size_t sz=0;
    char *block_size=getenv("SENSDECRYPT_BLOCK_COPY_SIZE");
    if (block_size != NULL) {
        sz = atoi(block_size);
    }
...
int main(int argc, char** argv) {
   ...
   while (1)
	...
	/* input dir is encrypted. prepare path string to initialize volume.fspf
	 * - volume.fspf must be located in the same directory as the input data.
	 * If the opposite behavior is required (input has plaintext, output
	 *  - encrypted), use real_out instead of real_in */
    char *volume_path = calloc(1, strlen(real_in) + strlen("/volume.fspf") + 1);
    if (!volume_path) {
        fprintf(stderr, "no memory when creating input path string \n");
        exit(EXIT_FAILURE);
    }
    strcpy(volume_path, real_in);
    strcpy(volume_path + strlen(real_in), "/volume.fspf");
